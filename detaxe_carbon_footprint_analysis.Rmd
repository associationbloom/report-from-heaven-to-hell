...
```
We observe an increase in fuel consumption (spanish + french) from 2009 to 2020
...
* Mean landings = `r format(round(catch_eu,0), big.mark = ",")` tons of tuna

* Amount of subsidies per landed ton of tuna (over the 2019-2021) = `r (data_stecf %>% ungroup () %>% filter(year %in% c(2019:2021)) %>% with_groups(year, summarise, fuel_tax_concessions = sum(fuel_tax_concessions)) %>% summarise (fuel_tax_concessions = mean(fuel_tax_concessions)) %>% pull (fuel_tax_concessions)*10^6 + (396462/3) + (35787928/3))/catch_eu`

NB : Over the 2019-2021 period, 396,462 euros were perceived by european tuna seiners under the EMFAF (European Maritime, Fisheries and Aquaculture Fund) and 35,787,928 euros under fishing agreements. 
The amounts of subsidies granted to European tuna vessels under fishing agreements have been estimated by cross-referencing the data on authorizations granted by the European Union to the various vessels with the amounts associated with the tuna agreements. The allocation was made in proportion to the
tonnage of vessels operating in African waters. 
...
* Mean landings = `r format(round(catch_maldives,0), big.mark = ",")` tons of tuna

* Amount of subsidies per landed ton of tuna = `r (169006/3)/catch_maldives`

NB: According to [this local press article] (https://mbr.mv/16120), Maldivian fishermen received 2,814,546 million Maldivian rupees between 2019 and 2021 in the form of fuel subsidies, equivalent to around 169,000 euros. 
